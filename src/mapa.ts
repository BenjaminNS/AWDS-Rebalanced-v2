import {terrenoJS} from './terreno'
import type { nombreTerreno } from './terreno'
import Konva from 'konva'
const {ListaTerrenos} = terrenoJS()
...
export function MapaJS(){
  class Casilla{
    ...
    constructor(tipo: nombreTerreno, dueno: number|null){
      if(ListaTerrenos[tipo] == undefined){
        ...
        if( ListaTerrenos[tipo].esPropiedad ){
          this.dueno=dueno;
        } else if ( !ListaTerrenos[tipo].esPropiedad && dueno != null ) {
  ...
  class Mapa{
    ...
    obtenerComandantesJugables = ():Set<number> =>{
      const setComandanteJugables = new Set()
      for (let i = 0; i < this.casillas.length; i++) {
        if( typeof this.casillas[i].dueno === 'number' ){
          setComandanteJugables.add(this.casillas[i].dueno)
        }
        if( this.casillas[i].unidad != null && typeof this.casillas[i].unidad.dueno === 'number' ){
          setComandanteJugables.add(this.casillas[i].unidad.dueno)
    ...
    esMapaValido = ():boolean => {
      ...
      for (let i = 0; i < this.casillas.length; i++) {
        ...
        if( typeof this.casillas[i].dueno === 'number' 
          ...
          setComandanteJugables.delete(this.casillas[i].dueno)
        }

        // O al menos una unidad
        if( this.casillas[i].unidad != null && typeof this.casillas[i].unidad.dueno === 'number' ){
          setComandanteJugables.delete(this.casillas[i].unidad.dueno)
        }
      }
      if( setComandanteJugables.size > 0 ){
  ...
  return {Casilla, Mapa, MapaSimple}
}
