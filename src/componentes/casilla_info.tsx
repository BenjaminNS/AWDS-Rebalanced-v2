...
export function CasillaInfo ({ casillaInfo, unidadInfo }:{casillaInfo: {estrellas:number, terreno: string}, unidadInfo: { hp:number, gasActual: number,
    gasMaxima: number, munPrincipal: number|null, munSecundaria: number|null, status: string|null
 }}){
  const { estrellas } = casillaInfo
  const { hp, gasActual, gasMaxima, munPrincipal, munSecundaria } = unidadInfo
  //   console.log(unidadInfo, casillaInfo)
  return (
    <>
